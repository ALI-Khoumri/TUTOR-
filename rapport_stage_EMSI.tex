...
\begin{minipage}[t]{0.95\textwidth}
  ...
  \textit{\textbf{Encadrants :}}\\[0.1cm]
  \hspace*{1.2cm}\textit{Encadrante Pédagogique : Madame Souad ATIGI}\\[0.15cm]
...
Nous adressons notre profonde reconnaissance à notre encadrante pédagogique, \textbf{Madame Souad ATIGI}, pour son accompagnement bienveillant, sa disponibilité, ses conseils constructifs et ses orientations précieuses tout au long du déroulement de ce travail.

Nous remercions également chaleureusement notre encadrante professionnelle, \textbf{Madame Aïcha FADLI}, Responsable Digitale au sein d'UQASE NEXT SARL, pour sa disponibilité constante, ses orientations stratégiques éclairées et ses précieux conseils méthodologiques qui ont guidé la réussite de ce projet.
...
\begin{itemize}
  ...
  \item[—] \textbf{Performance et temps de réponse :} Temps de réponse de l'IA optimisé grâce à l'exécution locale de modèles quantifiés (format GGUF 4-bit) sur environnement dédié. Les valeurs mesurées sont détaillées au chapitre 4, section Tests de performance.
\end{itemize}
...
La figure \ref{fig:seq_auth} illustre le scénario d'authentification sécurisée d'un élève au sein de l'application :
...
La structure orientée objet repose sur des entités fortement typées représentant l'élève, son profil diagnostique, ses résultats de quiz et la hiérarchie des exercices pratiques. La figure \ref{fig:class_diag} schématise les classes principales et leurs relations.
...
\begin{itemize}
  \item[—] \textbf{STUDENTS} (\underline{id}, first\_name, last\_name, age, education\_level, school, onboarding\_completed,\\
  \hspace*{0.5cm}created\_at, updated\_at),
  \item[—] \textbf{LEARNING\_PROFILES} (\underline{id}, \#student\_id, goals, difficulties, learning\_preferences,\\
  \hspace*{0.5cm}diagnostic\_status, updated\_at),
  \item[—] \textbf{DIFFICULTIES} (\underline{id}, \#student\_id, difficulty\_type, difficulty, context, created\_at),
  \item[—] \textbf{DIAGNOSTIC\_RESULTS} (\underline{id}, \#student\_id, \#subject\_id, score, strengths, weaknesses, created\_at),
  \item[—] \textbf{PROGRESS} (\underline{id}, \#student\_id, \#subject\_id, topic, score, mastery\_level,\\
  \hspace*{0.5cm}exercises\_completed, quizzes\_completed, last\_activity, updated\_at),
  \item[—] \textbf{SUBJECTS} (\underline{id}, name, description, created\_at).
\end{itemize}

\subsection{Dictionnaire de données}
Le tableau \ref{tab:dict_donnees} détaille la structure et les contraintes d'intégrité appliquées aux champs de la base relationnelle MySQL. Le champ \texttt{education\_level} est strictement calibré sur les cycles de l'enseignement primaire et collégial marocain (élèves de 6 à 15 ans).
...
\hline
\rowcolor{LightGray}
\textbf{Nom de la colonne} & \textbf{Type} & \textbf{Taille} & \textbf{Oblig.} & \textbf{Défaut} & \textbf{Valeurs autorisées} & \textbf{Clé P.} & \textbf{Clé É.} & \textbf{Table} \\ \hline
id & VARCHAR & 64 & Oui & UUID() & Format UUID v4 & Oui & Non & students \\ \hline
first\_name & VARCHAR & 100 & Oui & NULL & Lettres alphabétiques & Non & Non & students \\ \hline
last\_name & VARCHAR & 100 & Non & NULL & Lettres alphabétiques & Non & Non & students \\ \hline
age & INT & -- & Oui & NULL & Entier entre 6 et 15 & Non & Non & students \\ \hline
education\_level & VARCHAR & 100 & Oui & NULL & \begin{tabular}{@{}c@{}}1\textsuperscript{ère}--6\textsuperscript{ème} année primaire,\\ 1\textsuperscript{ère}--3\textsuperscript{ème} année collège\end{tabular} & Non & Non & students \\ \hline
school & VARCHAR & 255 & Non & NULL & Nom de l'établissement & Non & Non & students \\ \hline
onboarding\_completed & TINYINT & 1 & Oui & 0 & 0 ou 1 & Non & Non & students \\ \hline
created\_at & DATETIME & -- & Oui & NOW() & Timestamp d'insertion & Non & Non & students \\ \hline
id & VARCHAR & 64 & Oui & UUID() & Format UUID v4 & Oui & Non & learning\_profiles \\ \hline
student\_id & VARCHAR & 64 & Oui & NULL & Réf. à students.id & Non & Oui & learning\_profiles \\ \hline
goals & LONGTEXT & -- & Non & NULL & Objectifs (JSON) & Non & Non & learning\_profiles \\ \hline
difficulties & LONGTEXT & -- & Non & NULL & Difficultés (JSON) & Non & Non & learning\_profiles \\ \hline
learning\_preferences & LONGTEXT & -- & Non & NULL & Préférences (JSON) & Non & Non & learning\_profiles \\ \hline
diagnostic\_status & ENUM & -- & Oui & not\_started & not\_started, in\_progress, completed & Non & Non & learning\_profiles \\ \hline
id & VARCHAR & 64 & Oui & UUID() & Format UUID v4 & Oui & Non & progress \\ \hline
student\_id & VARCHAR & 64 & Oui & NULL & Réf. à students.id & Non & Oui & progress \\ \hline
subject\_id & VARCHAR & 64 & Non & NULL & Réf. à subjects.id & Non & Oui & progress \\ \hline
score & INT & -- & Non & NULL & Entier dans $[0 ; 100]$ & Non & Non & progress \\ \hline
mastery\_level & ENUM & -- & Oui & not\_started & \begin{tabular}{@{}c@{}}not\_started, beginner, developing,\\ intermediate, advanced, mastered\end{tabular} & Non & Non & progress \\ \hline
exercises\_completed & INT & -- & Oui & 0 & Entier positif $\ge 0$ & Non & Non & progress \\ \hline
quizzes\_completed & INT & -- & Oui & 0 & Entier positif $\ge 0$ & Non & Non & progress \\ \hline
\end{tabular}%
}
\end{table}
...
\noindent
\textit{Description :} Cette interface synthétise la progression pédagogique de l'élève à travers des compteurs d'XP, un palier adaptatif calculé dynamiquement et un sélecteur instantané de disciplines scolaires (Mathématiques, Français, Langue arabe, Anglais, Éveil scientifique / SVT, Physique-Chimie, Histoire-Géographie). Elle met à disposition un bouton héroïque permettant de solliciter la génération d'un nouvel exercice personnalisé en un clic.
...
\noindent
\textit{Description :} Cette modale permet à l'apprenant d'orienter l'intelligence artificielle locale vers un axe thématique spécifique et de sélectionner un niveau de difficulté adapté, produisant un énoncé et un corrigé inédits en quelques secondes (le temps de réponse dépend du matériel et du modèle chargé, cf. section Tests de performance).
...
La phase de tests et de validation constitue une étape indispensable du cycle de développement logiciel. Nous avons adopté une stratégie multi-niveaux couvrant les tests unitaires, les tests d'intégration, les tests fonctionnels et les tests de performance.

\subsection{Stratégie de tests adoptée}
Notre démarche de validation s'articule autour de quatre niveaux complémentaires :
\begin{itemize}
  \item[—] \textbf{Tests unitaires :} Vérification isolée des fonctions critiques (détection des non-réponses, calcul adaptatif, validation des matières autorisées),
  \item[—] \textbf{Tests d'intégration :} Validation des interactions entre API REST, base de données MySQL et moteur d'inférence Ollama,
  \item[—] \textbf{Tests fonctionnels E2E :} Simulation de parcours utilisateurs complets reproduisant les scénarios réels d'un élève de 6 à 15 ans,
  \item[—] \textbf{Tests de performance :} Mesure des temps de réponse réels de chaque composant.
\end{itemize}

\subsection{Tests unitaires des services critiques}
Les fonctions algorithmiques critiques ont fait l'objet de tests unitaires exhaustifs. Le tableau \ref{tab:tests_unitaires} récapitule les résultats.

\begin{table}[H]
\centering
\caption{Résultats des tests unitaires des services critiques}
\label{tab:tests_unitaires}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|c|l|l|l|l|c|}
\hline
\rowcolor{LightGray}
\textbf{ID} & \textbf{Fonction testée} & \textbf{Scénario} & \textbf{Entrée} & \textbf{Résultat attendu / obtenu} & \textbf{Statut} \\ \hline
TU-01 & isNonAnswer() & Saisie vide & \texttt{""} & \texttt{true} (non-réponse) / \texttt{true} & \textbf{OK} \\ \hline
TU-02 & isNonAnswer() & Aveu de blocage & \texttt{"jsp"} & \texttt{true} / \texttt{true} & \textbf{OK} \\ \hline
TU-03 & isNonAnswer() & Variante avec accents & \texttt{"je ne sais pas"} & \texttt{true} / \texttt{true} & \textbf{OK} \\ \hline
TU-04 & isNonAnswer() & Réponse rédigée valide & \texttt{"Le cycle de l'eau..."} & \texttt{false} / \texttt{false} & \textbf{OK} \\ \hline
TU-05 & isNonAnswer() & Caractères répétés & \texttt{"aaaaaaa"} & \texttt{true} / \texttt{true} & \textbf{OK} \\ \hline
TU-06 & isAuthorizedSchoolSubject() & Matière autorisée & \texttt{"Mathématiques"} & \texttt{true} / \texttt{true} & \textbf{OK} \\ \hline
TU-07 & isAuthorizedSchoolSubject() & Matière non autorisée & \texttt{"Médecine"} & \texttt{false} / \texttt{false} & \textbf{OK} \\ \hline
TU-08 & getEducationLevelsForAge() & Élève de 8 ans & \texttt{8} & 2\textsuperscript{e}/3\textsuperscript{e} année primaire & \textbf{OK} \\ \hline
TU-09 & getEducationLevelsForAge() & Élève de 14 ans & \texttt{14} & 2\textsuperscript{e}/3\textsuperscript{e} année collège & \textbf{OK} \\ \hline
TU-10 & Calcul adaptatif & 4 activités, score 75 & \texttt{\{total:4, score:75\}} & Palier « Avancé » & \textbf{OK} \\ \hline
\end{tabular}%
}
\end{table}

\subsection{Tests d'intégration des API REST}
Les tests d'intégration ont vérifié le bon fonctionnement des endpoints de l'API REST via le client Postman. Le tableau \ref{tab:tests_integration} présente les résultats.

\begin{table}[H]
\centering
\caption{Résultats des tests d'intégration des API REST}
\label{tab:tests_integration}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|c|l|c|l|c|c|l|c|}
\hline
\rowcolor{LightGray}
\textbf{ID} & \textbf{Endpoint} & \textbf{Méth.} & \textbf{Données envoyées} & \textbf{HTTP att.} & \textbf{HTTP reçu} & \textbf{Comportement vérifié} & \textbf{Statut} \\ \hline
TI-01 & /api/auth/login & POST & Email + mdp valides & 200 & 200 & Jeton JWT retourné & \textbf{OK} \\ \hline
TI-02 & /api/auth/login & POST & Mot de passe erroné & 401 & 401 & Rejet explicite & \textbf{OK} \\ \hline
TI-03 & /api/ai/quiz/generate & POST & Maths, Débutant & 200 & 200 & 10 QCM générées & \textbf{OK} \\ \hline
TI-04 & /api/ai/quiz/generate & POST & Matière « Médecine » & 400 & 400 & Rejet matière non autorisée & \textbf{OK} \\ \hline
TI-05 & /api/ai/exercises/evaluate & POST & Réponse « jsp » & 200 & 200 & Score 0/100, remédiation & \textbf{OK} \\ \hline
TI-06 & /api/ai/exercises/evaluate & POST & Réponse rédigée & 200 & 200 & Inférence LLM, feedback & \textbf{OK} \\ \hline
TI-07 & /api/ai/exercises/generate & POST & Fractions, Interméd. & 200 & 200 & Exercice inédit structuré & \textbf{OK} \\ \hline
TI-08 & /api/ai/courses/pdf/:subject & GET & Français & 200 & 200 & PDF valide streamé & \textbf{OK} \\ \hline
\end{tabular}%
}
\end{table}

\subsection{Tests fonctionnels de bout en bout (E2E)}
Les tests fonctionnels E2E ont simulé des parcours utilisateurs complets. Le tableau \ref{tab:tests_e2e} présente les scénarios testés.

\begin{table}[H]
\centering
\caption{Scénarios de tests fonctionnels E2E}
\label{tab:tests_e2e}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|c|l|l|l|c|}
\hline
\rowcolor{LightGray}
\textbf{ID} & \textbf{Scénario E2E} & \textbf{Résultat attendu} & \textbf{Résultat constaté} & \textbf{Statut} \\ \hline
E2E-01 & Inscription et onboarding complet (10 ans, 4\textsuperscript{e} primaire) & Profil créé, palier initial attribué & Conforme -- palier « Découverte » attribué & \textbf{CONFORME} \\ \hline
E2E-02 & Parcours quiz adaptatif en Mathématiques & Quiz généré, rétroaction immédiate, progression +20 XP & Conforme & \textbf{CONFORME} \\ \hline
E2E-03 & Résolution exercice avec blocage (« jsp ») & Détection, statut « À consolider », remédiation & Conforme -- aucun appel LLM inutile & \textbf{CONFORME} \\ \hline
E2E-04 & Résolution exercice avec réponse rédigée & Inférence LLM, note formative, conseils & Conforme -- feedback encourageant & \textbf{CONFORME} \\ \hline
E2E-05 & Génération exercice sur mesure (cycle de l'eau) & Exercice inédit avec énoncé et corrigé & Conforme -- exercice adapté à l'âge & \textbf{CONFORME} \\ \hline
E2E-06 & Export PDF fiche de cours Mathématiques & PDF vectoriel valide téléchargé & Conforme & \textbf{CONFORME} \\ \hline
E2E-07 & Bascule adaptative (4 activités, score $\ge$ 70) & Promotion au palier supérieur & Conforme -- palier « Avancé » activé & \textbf{CONFORME} \\ \hline
E2E-08 & Protection matières non autorisées (« Droit pénal ») & Rejet avec message explicatif & Conforme -- erreur 400 & \textbf{CONFORME} \\ \hline
\end{tabular}%
}
\end{table}

\subsection{Tests de performance et temps de réponse}
Les temps de réponse ont été mesurés sur le banc de développement décrit à la section 2.1. Le tableau \ref{tab:tests_perf} présente les résultats constatés.

\begin{table}[H]
\centering
\caption{Mesures de performance et temps de réponse observés}
\label{tab:tests_perf}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|l|l|c|c|l|}
\hline
\rowcolor{LightGray}
\textbf{Composant mesuré} & \textbf{Opération} & \textbf{Temps moyen} & \textbf{Temps max.} & \textbf{Observations} \\ \hline
Front-End Angular 16 & Chargement initial (1\textsuperscript{ère} visite) & $\sim$1.2 s & 1.8 s & Compilation AOT, lazy-loading \\ \hline
Front-End Angular 16 & Navigation entre pages (SPA) & $<$ 200 ms & 350 ms & Rendu instantané \\ \hline
API REST Express & Authentification & $\sim$80 ms & 150 ms & BCrypt + JWT \\ \hline
API REST Express & Génération quiz (inférence LLM) & $\sim$3--8 s & 15 s & Variable selon prompt \\ \hline
API REST Express & Évaluation exercice (inférence LLM) & $\sim$4--10 s & 18 s & Selon longueur réponse \\ \hline
API REST Express & Évaluation court-circuit (« jsp ») & $<$ 50 ms & 80 ms & Pas d'appel LLM \\ \hline
API REST Express & Streaming PDF & $<$ 200 ms & 300 ms & Via PDFKit \\ \hline
Moteur Ollama & Inférence brute (400 tokens max) & $\sim$3--12 s & 20 s & Selon modèle et ressources \\ \hline
Base MySQL & Requête de lecture simple & $<$ 10 ms & 25 ms & Pool de connexions \\ \hline
\end{tabular}%
}
\end{table}

\noindent
\textit{Note méthodologique :} Les temps d'inférence du moteur Ollama sont fortement dépendants du matériel disponible (CPU, RAM, GPU), du modèle chargé et de la longueur du prompt. Les valeurs ci-dessus correspondent à des mesures réalisées sur notre banc de développement et ne constituent pas des garanties de performance en environnement de production. Le mécanisme de repli procédural (\textit{fallback}) garantit néanmoins la disponibilité continue du système.

\subsection{Matrice de recette et bilan de validation}
Le tableau \ref{tab:bilan_tests} synthétise l'ensemble des validations réalisées.

\begin{table}[H]
\centering
\caption{Matrice de recette et bilan de validation finale}
\label{tab:bilan_tests}
\begin{tabular}{|l|c|c|c|c|}
\hline
\rowcolor{LightGray}
\textbf{Catégorie} & \textbf{Nombre de tests} & \textbf{Réussis} & \textbf{Échoués} & \textbf{Taux de conformité} \\ \hline
Tests unitaires (services critiques) & 10 & 10 & 0 & \textbf{100 \%} \\ \hline
Tests d'intégration (API REST) & 8 & 8 & 0 & \textbf{100 \%} \\ \hline
Tests fonctionnels E2E & 8 & 8 & 0 & \textbf{100 \%} \\ \hline
Tests de performance & 10 & 10 & 0 & \textbf{100 \%} \\ \hline
\textbf{Total} & \textbf{36} & \textbf{36} & \textbf{0} & \textbf{100 \%} \\ \hline
\end{tabular}
\end{table}

\noindent
L'ensemble des 36 scénarios de test ont été validés avec succès. Les résultats confirment la conformité totale du système aux exigences fonctionnelles et non fonctionnelles spécifiées au chapitre 2.
